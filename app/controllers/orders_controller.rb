class OrdersController < ApplicationController
  ...
  def is_paid
    ...
    File.open("#{Rails.root}/log/is_paid.log", "a+") do |file|
      ...
      file.syswrite(%(#{Time.now.iso8601}: #{params_copy.delete('CheckMacValue')} \n---------------------------------------------\n\n))
      raw_data = params_copy.sort_by { |k,v| k.downcase }.map do |x, y|
        "#{x}=#{y}"
      end.join('&')
      file.syswrite(%(#{Time.now.iso8601}: #{raw_data} \n---------------------------------------------\n\n))
      ...
      file.syswrite(%(#{Time.now.iso8601}: #{hash_raw_data} \n---------------------------------------------\n\n))
      url_encode_data = OffsitePayments::Integrations::Allpay::Helper.url_encode(hash_raw_data)
      file.syswrite(%(#{Time.now.iso8601}: #{url_encode_data} \n---------------------------------------------\n\n))
      file.syswrite(%(#{Time.now.iso8601}: #{url_encode_data.downcase!} \n---------------------------------------------\n\n))
      file.syswrite(%(#{Time.now.iso8601}: #{Digest::SHA256.hexdigest(url_encode_data)} \n---------------------------------------------\n\n))
    end
